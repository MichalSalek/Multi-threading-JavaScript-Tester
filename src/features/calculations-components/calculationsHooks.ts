...
import {
	IWorkerReadyState, IWorkerWorkState,
...
export const useSpecificWorkerStatus = (command: UseSpecificWorkerStatusCommandEnum, workerKey: WorkerKeyType): UseSpecificWorkerStatusType => {
	...
	useEffect(() => {
		setReturnedSpecificWorkerStatus(() => {
			switch (command) {
			case UseSpecificWorkerStatusCommandEnum.ready: return isReady
			case UseSpecificWorkerStatusCommandEnum.working: return isWorking
			}
		})
		return () => undefined
