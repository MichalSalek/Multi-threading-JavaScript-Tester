...
const CalculationResultsFloatingMolecule = (): JSX.Element => {
	...
	const allWorkersActuallyWorkAmount: WorkersAmountStateType = useAppSelector(selectActuallyWorkingWorkersAmount)
		...
		<DraggableItemComposition initialPosition={{x: 500, y: 300}}>
			<section className={`${scss.host} display-inline-block`}>
				<h4>Calculation results:</h4> <span>Show all, even currently deactivated: </span><input
					type={'checkbox'} checked={showDeactivated}
					onChange={(event => setShowDeactivated(event.currentTarget.checked))}/>
				<h5>Required number of workers: {workerExpectedAmount.amount}</h5>
				<h5>Workers who actually work: {allWorkersActuallyWorkAmount.amount}</h5>
				...
				<ol className={[scss.listGrid, (() => (showDeactivated || workerExpectedAmount.amount > 0) ? '' : 'display-none')()].join(' ')}>
